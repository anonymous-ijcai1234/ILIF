...
class PreActBlock(nn.Module):
    ...
    def forward(self, x):
        x = self.relu1(self.bn1(x)) # 128,64,32,32
        out = self.conv1(x)
        x_temp = self.relu2(self.bn2(out))
        out = self.conv2(self.dropout(x_temp))
...
class PreActBottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv1(x)
        x_temp = self.relu2(self.bn2(out))
        out = self.conv2(x_temp)
        x_temp = self.relu3(self.bn3(out))
        out = self.conv3(self.dropout(x_temp))
        out = out + self.shortcut(x)
